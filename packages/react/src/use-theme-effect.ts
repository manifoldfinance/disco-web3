...
import { theme as lightTheme, darkTheme } from '@disco3/core';
...
export function useThemeEffect(theme: 'light' | 'dark' = 'light') {
  const themeClass =
    theme === 'light' ? lightTheme.className : darkTheme.className;
  ...
  const isDifferentTheme =
    hasModeClass && !htmlTag?.classList.contains(themeClass);
