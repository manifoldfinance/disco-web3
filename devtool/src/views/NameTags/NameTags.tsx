    ...
    () =>
      nameTags
        ...
        .map((tag, i, array) => ({
          ...
          shadowed: array.some(
            (x, j) =>
              j > i && x.address.toLowerCase() === tag.address.toLowerCase(),
          ),
        })),
    [nameTags],
  );
  ...
  return (
    <Page name="nameTags" onNavigate={onNavigate}>
      <Wrapper>
        ...
        <Text>
          Name tags are used for identifying Ethereum addresses. Instead of
          remembering the hex values you can assign a human readable name to
          them. This names are used across this extension. Some addresses and
          names are added automatically (e.g. connected accounts or multicall
          contracts). Those can be changed at any point.
        </Text>
        ...
        <Table>
          <tbody>
            {displayed
              .map((tag, i) => (
                <Row key={i}>
                  <AddressCell className={tag.shadowed ? 'shadowed' : ''}>
                    {tag.address}
                  </AddressCell>
                  <NameCell className={tag.shadowed ? 'shadowed' : ''}>
                    {tag.name}
                  </NameCell>
                  <Cell>
