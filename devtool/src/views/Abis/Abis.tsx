  ...
  return (
    <Page name="abis" onNavigate={onNavigate}>
      <Wrapper>
        ...
        <Text>
          ABIs are used to parse call data. Adding ABIs from your contracts will
          allow you to easily inspect method calls that your application is
          making.
        </Text>
        ...
        <AbiList>
          {getAbis(abis).map((entry, i) => (
            <AbiItem key={i} className={entry.disabled ? 'disabled' : ''}>
              <Signature className={entry.shadowed ? 'shadowed' : ''}>
                {entry.code}
              </Signature>
              {!entry.disabled && (
                <Remove onClick={() => remove(i)}>Remove</Remove>
              )}
            </AbiItem>
          ))}
        </AbiList>
