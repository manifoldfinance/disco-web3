...
export function EventProvider({ children }: Props) {
  ...
  function onMessage(message: Message) {
    ...
    if (message.source === 'usedapp-hook') {
      ...
      message.source === 'usedapp-content' &&
      message.payload.type === 'REPLAY'
    ) {
  ...
  function handle(message: Message) {
    if (message.payload.type === 'ACCOUNT_CHANGED' && message.payload.address) {
      tag(
        ...
        `User ${message.payload.address.substring(2, 6)}`,
      );
    } else if (
      message.payload.type === 'NETWORK_CHANGED' &&
      message.payload.multicallAddress
    ) {
