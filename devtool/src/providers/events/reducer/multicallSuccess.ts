...
export function multicallSuccess(
  state: State,
  message: HookMessage<MulticallSuccessPayload>,
): State {
  ...
  for (const entry of previousEntries) {
    const corresponding = currentEntries.find(
      (x) =>
        x.address.toLowerCase() === entry.address.toLowerCase() &&
        x.data === entry.data,
    );
    if (corresponding?.value !== entry.value) {
      if (
        state.blockNumbers[network] === message.payload.blockNumber &&
        corresponding?.value === undefined
      ) {
