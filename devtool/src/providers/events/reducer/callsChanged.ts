...
export function callsChanged(
  state: State,
  message: HookMessage<CallsChangedPayload>,
): State {
  ...
  for (const call of message.payload.calls) {
    const existing = state.calls.find(
      (x) =>
        x.address.toLowerCase() === call.address.toLowerCase() &&
        x.data === call.data,
    );
    if (existing) {
  ...
  const network =
    message.payload.chainId !== undefined
      ? chainIdToNetwork(message.payload.chainId)
      : undefined;
  return {
    ...state,
