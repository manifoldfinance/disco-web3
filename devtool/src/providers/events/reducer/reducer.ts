  ...
  switch (message.payload.type) {
    ...
    case 'NETWORK_CHANGED':
      return networkChanged(
        state,
        message as HookMessage<NetworkChangedPayload>,
      );
    case 'BLOCK_NUMBER_CHANGED':
      return blockNumberChanged(
        state,
        message as HookMessage<BlockNumberChangedPayload>,
      );
    case 'ACCOUNT_CHANGED':
      return accountChanged(
        state,
        message as HookMessage<AccountChangedPayload>,
      );
    case 'CALLS_CHANGED':
      return callsChanged(state, message as HookMessage<CallsChangedPayload>);
    case 'MULTICALL_SUCCESS':
      return multicallSuccess(
        state,
        message as HookMessage<MulticallSuccessPayload>,
      );
    case 'MULTICALL_ERROR':
      return multicallError(
        state,
        message as HookMessage<MulticallErrorPayload>,
      );
    case 'GENERIC_ERROR':
