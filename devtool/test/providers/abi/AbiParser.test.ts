...
describe('AbiParser', () => {
  ...
  describe('call data', () => {
    ...
    it('named and unnamed arguments', () => {
      const result = encodeAndDecode(
        'function mixed(uint256 a, bool, bool c, uint256)',
        [1, false, true, 2],
      );
      expect(result).to.deep.equal([
        {
    ...
    describe('types', () => {
      ...
      for (const testCase of testCases) {
        it(testCase.inputType, () => {
          const result = encodeAndDecode(
            `function foo(${testCase.inputType})`,
            [testCase.inputValue],
          );
          expect(result).to.deep.equal([
            {
  ...
  describe('call result', () => {
    ...
    it('multiple results', () => {
      const result = encodeAndDecode('function foo() returns (uint, bool)', [
        1,
        false,
      ]);
      expect(result).to.deep.equal({
    ...
    it('named results', () => {
      const result = encodeAndDecode(
        'function foo() returns (uint a, bool b)',
        [1, false],
      );
      expect(result).to.deep.equal({
        type: 'tuple',
