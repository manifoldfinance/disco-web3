...
import type {
  ...
  State,
  StateUpdatedEvent,
} from '../../../src/providers/events/State';
...
describe('reducer', () => {
  ...
  describe('network changed', () => {
    ...
    it('does nothing if connected to the same network', () => {
      const result = stateAfter(
        ...
        makeNetworkChangedMessage('13:14:16', 1),
      );
    ...
    it('is disconnected by default', () => {
      const result = stateAfter(
        makeNetworkChangedMessage('13:14:15', undefined),
      );
    ...
    it('preserves past events when connected', () => {
      const result = stateAfter(
        makeNetworkChangedMessage('13:14:15', 1),
        makeNetworkChangedMessage('13:14:16', 2),
      );
  ...
  describe('block number changed', () => {
    it('adds a block found event', () => {
      const result = stateAfter(
        makeBlockNumberChangedMessage('12:00:00.123', 1, 123456),
      );
      const expected: State = {
        ...STATE_WITH_INIT,
        events: [
          INIT_EVENT,
          makeBlockFoundEvent('+00:00.123', 'Mainnet', 123456),
        ],
      };
      expect(result).to.deep.equal(expected);
    });
  ...
  describe('account changed', () => {
    ...
    it('is disconnected by default', () => {
      const result = stateAfter(
        makeAccountChangedMessage('13:14:15', undefined),
      );
      expect(result).to.deep.equal(STATE_WITH_INIT);
    });
  ...
  describe('multicall success', () => {
    ...
    const entry = (address: string, data: string, value: string) => ({
      address,
      data,
      value,
    });
  ...
  describe('generic error', () => {
    it('adds the event to the list of messages', () => {
      ...
      const expected: State = {
        ...
        events: [
          INIT_EVENT,
          makeErrorEvent('+00:00.123', 'Foo'),
          makeErrorEvent('+14:15.456', 'Bar'),
        ],
      };
      expect(result).to.deep.equal(expected);
    });
...
function stateAfter(...messages: Message[]) {
  ...
  return messages
    .map((x) => JSON.parse(JSON.stringify(x)) as Message)
    .reduce(reducer, STATE_WITH_INIT);
}
...
function makeNetworkChangedMessage(
  time: string,
...
function makeBlockNumberChangedMessage(
  time: string,
  chainId: number,
  blockNumber: number,
): Message {
...
function makeAccountChangedMessage(
  time: string,
...
function makeCallsChangedMessage(
  time: string,
  chainId: number,
  calls: ChainCall[],
): Message {
...
function makeMulticallErrorMessage(
  time: string,
  options: Omit<MulticallErrorPayload, 'type'>,
): Message {
  return {
    source: 'usedapp-hook',
    timestamp: toTimestamp(time),
...
function makeMulticallSuccessMessage(
  time: string,
  options: Omit<MulticallSuccessPayload, 'type'>,
): Message {
  return {
    source: 'usedapp-hook',
    timestamp: toTimestamp(time),
...
function makeNetworkConnectedEvent(
  time: string,
  chainId: number,
  network: string,
): Event {
...
function makeFetchErrorEvent(
  time: string,
  options: Omit<FetchErrorEvent, 'type' | 'time'>,
): Event {
  return {
    type: 'FETCH_ERROR',
    time,
...
function makeStateUpdatedEvent(
  time: string,
  options: Omit<StateUpdatedEvent, 'type' | 'time'>,
): Event {
  return {
    type: 'STATE_UPDATED',
    time,
